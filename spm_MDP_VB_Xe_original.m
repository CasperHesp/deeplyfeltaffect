...
try, betae = MDP(1).betae; catch, betae = 1;    end
        ...
        if ~HMM || T == t
            ...
            if Np(m) > 1
                ...
                for i = 1:Ni
                    ...
                    %------------------------------------------------------
                    %display(w{m}(t))
                    qu = spm_softmax(we{m}(t)*qE{m}(p{m}) + w{m}(t)*Q(p{m}) + F(p{m}));
                    pu = spm_softmax(we{m}(t)*qE{m}(p{m}) + w{m}(t)*Q(p{m}));
...
for m = 1:size(MDP,1)
    ...
    if isfield(MDP,'d')
        for f = 1:Nf(m)
            i = MDP(m).d{f} > 0;
            MDP(m).d{f}(i) = MDP(m).d{f}(i) + MDP(m).ksi*X{m,f}(i,1);
        end
    end
    ...
    MDP(m).w  = w{m};         % posterior expectations of precision (G)
    MDP(m).we  = we{m};       % posterior expectations of precision (E)
